...
static struct pico_proto_rr proto_rr_datalink = {&Datalink_proto_tree, NULL,
                                                 NULL};
static struct pico_proto_rr proto_rr_network = {&Network_proto_tree, NULL,
                                                NULL};
static struct pico_proto_rr proto_rr_transport = {&Transport_proto_tree, NULL,
                                                  NULL};
...
static struct pico_tree_node *roundrobin_init(struct pico_proto_rr *rr,
                                              int direction) {
  struct pico_tree_node *next_node = NULL;
  /* Initialization (takes place only once) */
  if (rr->node_in == NULL) rr->node_in = pico_tree_firstNode(rr->t->root);

  if (rr->node_out == NULL) rr->node_out = pico_tree_firstNode(rr->t->root);
...
static void roundrobin_end(struct pico_proto_rr *rr, int direction,
...
static int pico_protocol_generic_loop(struct pico_proto_rr *rr, int loop_score,
                                      int direction) {
