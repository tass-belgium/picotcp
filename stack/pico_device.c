...
static struct pico_devices_rr_info Devices_rr_info = {NULL, NULL};
...
static struct pico_ipv6_link *pico_6lowpan_link_add(
    struct pico_device *dev, const struct pico_ip6 *prefix) {
  struct pico_ip6 netmask64 = {{0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
...
static int pico_6lowpan_store_info(struct pico_device *dev,
                                   const uint8_t *mac) {
...
static void device_init_ipv6_final(struct pico_device *dev,
                                   struct pico_ip6 *linklocal) {
  dev->hostvars.basetime = PICO_ND_REACHABLE_TIME;
  /* RFC 4861 $6.3.2 value between 0.5 and 1.5 times basetime */
  dev->hostvars.reachabletime =
      ((5 + (pico_rand() % 10)) * PICO_ND_REACHABLE_TIME) / 10;
...
static int device_init_mac(struct pico_device *dev, const uint8_t *mac) {
  ...
  struct pico_ip6 linklocal = {{0xfe, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                0xaa, 0xaa, 0xaa, 0xff, 0xfe, 0xaa, 0xaa,
                                0xaa}};
#endif
  ...
  if (0) {
    ...
    if (pico_6lowpan_store_info(dev, mac)) return -1;
  }
#endif
  else {
...
int pico_device_ipv6_random_ll(struct pico_device *dev) {
  ...
  struct pico_ip6 linklocal = {{0xfe, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                0xaa, 0xaa, 0xaa, 0xff, 0xfe, 0xaa, 0xaa,
                                0xaa}};
  struct pico_ip6 netmask6 = {{0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
...
int pico_device_init(struct pico_device *dev, const char *name,
  ...
  if (len > MAX_DEVICE_NAME) len = MAX_DEVICE_NAME;
  ...
  if (!dev->mtu) dev->mtu = PICO_DEVICE_DEFAULT_MTU;

#ifdef PICO_SUPPORT_6LOWPAN
  if (PICO_DEV_IS_6LOWPAN(dev) && LL_MODE_ETHERNET == dev->mode) return -1;
#endif
...
void pico_device_destroy(struct pico_device *dev) {
  ...
  if (!dev->mode && dev->eth) PICO_FREE(dev->eth);
...
static int devloop(struct pico_device *dev, int loop_score, int direction) {
  /* If device supports interrupts, read the value of the condition and trigger
   * the dsr */
  if (direction == PICO_LOOP_DIR_IN) {
    loop_score = check_dev_serve_interrupt(dev, loop_score);

    /* If device supports polling, give control. Loop score is managed
     * internally, remaining loop points are returned. */
...
static void pico_dev_roundrobin_end(int direction,
                                    struct pico_tree_node *last) {
  if (direction == PICO_LOOP_DIR_IN)
...
int pico_devices_loop(int loop_score, int direction) {
  ...
  if (!next_node) return loop_score;
