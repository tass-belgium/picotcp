...
static int socket_cmp_ipv6(struct pico_socket *a, struct pico_socket *b) {
  ...
  if (!is_sock_ipv6(a) || !is_sock_ipv6(b)) return 0;

  if ((memcmp(a->local_addr.ip6.addr, PICO_IP6_ANY, PICO_SIZE_IP6) == 0) ||
      (memcmp(b->local_addr.ip6.addr, PICO_IP6_ANY, PICO_SIZE_IP6) == 0))
    ret = 0;
  else
...
static int socket_cmp_ipv4(struct pico_socket *a, struct pico_socket *b) {
  ...
  (void)a;
  (void)b;
  if (!is_sock_ipv4(a) || !is_sock_ipv4(b)) return 0;

#ifdef PICO_SUPPORT_IPV4
  if ((a->local_addr.ip4.addr == PICO_IP4_ANY) ||
      (b->local_addr.ip4.addr == PICO_IP4_ANY))
    ret = 0;
  else
...
static int socket_cmp_remotehost(struct pico_socket *a, struct pico_socket *b) {
  ...
  if (is_sock_ipv6(a))
    ret =
        memcmp(a->remote_addr.ip6.addr, b->remote_addr.ip6.addr, PICO_SIZE_IP6);
  else
...
static int socket_cmp_addresses(struct pico_socket *a, struct pico_socket *b) {
  ...
  if (ret == 0) ret = socket_cmp_ipv4(a, b);

  /* Sort by remote host */
  if (ret == 0) ret = socket_cmp_remotehost(a, b);
...
static int socket_cmp(void *ka, void *kb) {
  ...
  if (ret == 0) ret = socket_cmp_addresses(a, b);

  /* And finally by remote port. The two sockets are coincident if the quad is
   * the same. */
  if (ret == 0) ret = b->remote_port - a->remote_port;

  return ret;
}

#define INIT_SOCKPORT \
  { {&LEAF, socket_cmp}, 0, 0 }
...
static int pico_port_in_use_with_this_ipv4_address(struct pico_sockport *sp,
                                                   struct pico_ip4 ip) {
  if (sp) {
    ...
    pico_tree_foreach(idx, &sp->socks) {
      ...
      if (s->net == &pico_proto_ipv4) {
        ...
        if ((s_local->addr == PICO_IPV4_INADDR_ANY) ||
            (s_local->addr == ip.addr)) {
          return 1;
        }
      }
    }
...
#ifdef PICO_SUPPORT_IPV6
static int pico_port_in_use_with_this_ipv6_address(struct pico_sockport *sp,
                                                   struct pico_ip6 ip) {
  if (sp) {
    ...
    pico_tree_foreach(idx, &sp->socks) {
      ...
      if (s->net == &pico_proto_ipv6) {
        ...
        if ((pico_ipv6_is_unspecified(s_local->addr)) ||
            (!memcmp(s_local->addr, ip.addr, PICO_SIZE_IP6))) {
          return 1;
        }
      }
    }
...
static int pico_generic_port_in_use(uint16_t proto, uint16_t port,
                                    struct pico_sockport *sp, void *addr,
                                    void *net) {
#ifdef PICO_SUPPORT_IPV4
...
int8_t pico_socket_add(struct pico_socket *s) {
  ...
  if (!sp) {
    /* dbg("Creating sockport..%04x\n", s->local_port); / * In comment due to
     * spam during test * / */
...
static void pico_socket_check_empty_sockport(struct pico_socket *s,
                                             struct pico_sockport *sp) {
...
int8_t pico_socket_del(struct pico_socket *s) {
  ...
  if (!pico_timer_add((pico_time)10, socket_garbage_collect, s)) {
    dbg("SOCKET: Failed to start garbage collect timer, doing garbage "
        "collection now\n");
    PICOTCP_MUTEX_UNLOCK(Mutex);
...
static void pico_socket_update_tcp_state(struct pico_socket *s,
                                         uint16_t tcp_state) {
  if (tcp_state) {
    s->state &= 0x00FF;
    s->state |= tcp_state;
...
int8_t pico_socket_alter_state(struct pico_socket *s, uint16_t more_states,
  ...
  if (less_states & PICO_SOCKET_STATE_BOUND) return pico_socket_del(s);
...
static int pico_socket_transport_deliver(struct pico_protocol *p,
                                         struct pico_sockport *sp,
                                         struct pico_frame *f) {
#ifdef PICO_SUPPORT_TCP
  if (p->proto_number == PICO_PROTO_TCP) return pico_socket_tcp_deliver(sp, f);

#endif

#ifdef PICO_SUPPORT_UDP
  if (p->proto_number == PICO_PROTO_UDP) return pico_socket_udp_deliver(sp, f);
...
static int pico_socket_deliver(struct pico_protocol *p, struct pico_frame *f,
                               uint16_t localport) {
  struct pico_sockport *sp = NULL;
...
#ifdef PICO_SUPPORT_IPV4
  if (family == PICO_PROTO_IPV4) s->net = &pico_proto_ipv4;

#endif

#ifdef PICO_SUPPORT_IPV6
  if (family == PICO_PROTO_IPV6) s->net = &pico_proto_ipv6;

#endif

  if (s->net == NULL) return -1;
...
static struct pico_socket *pico_socket_transport_open(uint16_t proto,
  ...
  (void)family;
#ifdef PICO_SUPPORT_UDP
  if (proto == PICO_PROTO_UDP) s = pico_socket_udp_open();

#endif

#ifdef PICO_SUPPORT_TCP
  if (proto == PICO_PROTO_TCP) s = pico_socket_tcp_open(family);
...
struct pico_socket *MOCKABLE
pico_socket_open(uint16_t net, uint16_t proto,
                 void (*wakeup)(uint16_t ev, struct pico_socket *)) {
  struct pico_socket *s = NULL;
...
static void pico_socket_clone_assign_address(struct pico_socket *s,
                                             struct pico_socket *facsimile) {
#ifdef PICO_SUPPORT_IPV4
...
static int pico_socket_transport_read(struct pico_socket *s, void *buf,
                                      int len) {
  if (PROTO(s) == PICO_PROTO_UDP) {
...
static void *pico_socket_sendto_get_ip4_src(struct pico_socket *s,
                                            struct pico_ip4 *dst) {
  ...
  if (src4->addr != PICO_IPV4_INADDR_ANY) s->local_addr.ip4.addr = src4->addr;
...
static void *pico_socket_sendto_get_ip6_src(struct pico_socket *s,
                                            struct pico_ip6 *dst) {
  ...
  if ((s->state & PICO_SOCKET_STATE_CONNECTED)) {
    ...
    if (!pico_ipv6_is_unspecified(src6->addr)) s->local_addr.ip6 = *src6;
  }
...
static int pico_socket_sendto_dest_check(struct pico_socket *s, void *dst,
                                         uint16_t port) {
   ...
   * the destination is automatically assigned to the currently connected
   * endpoint. This check will ensure that there is no mismatch when sendto() is
   * called directly on a connected socket
   */
  if ((s->state & PICO_SOCKET_STATE_CONNECTED) != 0) {
...
static int pico_socket_sendto_initial_checks(struct pico_socket *s,
...
static struct pico_remote_endpoint *pico_socket_sendto_destination_ipv4(
    struct pico_socket *s, struct pico_ip4 *dst, uint16_t port) {
...
static struct pico_remote_endpoint *pico_socket_sendto_destination_ipv6(
    struct pico_socket *s, struct pico_ip6 *dst, uint16_t port) {
  struct pico_remote_endpoint *ep = NULL;
  (void)s;
  (void)dst;
  (void)port;
...
static struct pico_remote_endpoint *pico_socket_sendto_destination(
    struct pico_socket *s, void *dst, uint16_t port) {
  struct pico_remote_endpoint *ep = NULL;
...
static int pico_socket_sendto_transport_offset(struct pico_socket *s) {
  int header_offset = -1;
#ifdef PICO_SUPPORT_TCP
  if (PROTO(s) == PICO_PROTO_TCP) header_offset = pico_tcp_overhead(s);

#endif

#ifdef PICO_SUPPORT_UDP
  if (PROTO(s) == PICO_PROTO_UDP) header_offset = sizeof(struct pico_udp_hdr);

#endif
  return header_offset;
}

static struct pico_remote_endpoint *pico_socket_set_info(
    struct pico_remote_endpoint *ep) {
...
static int pico_socket_xmit_one(struct pico_socket *s, const void *buf,
                                const int len, void *src,
                                struct pico_remote_endpoint *ep,
  ...
  if (msginfo) {
    dev = msginfo->dev;
           ...
           pico_ipv6_is_multicast(&ep->remote_addr.ip6.addr[0])) {
...
#ifdef PICO_SUPPORT_IPV4FRAG
static void pico_socket_xmit_first_fragment_setup(struct pico_frame *f,
                                                  int space, int hdr_offset) {
...
static void pico_socket_xmit_next_fragment_setup(struct pico_frame *f,
                                                 int hdr_offset,
                                                 int total_payload_written,
                                                 int len) {
  /* no transport header in fragmented IP */
  f->payload = f->transport_hdr;
  /* set offset in octets */
  f->frag = (uint16_t)((total_payload_written + (uint16_t)hdr_offset) >>
                       3u); /* first fragment had a header offset */
...
static int pico_socket_xmit_fragments(struct pico_socket *s, const void *buf,
                                      const int len, void *src,
                                      struct pico_remote_endpoint *ep,
                                      struct pico_msginfo *msginfo) {
  ...
  while (total_payload_written < len) {
    ...
    if (space >
        len - total_payload_written) /* update space for last fragment */
      space = len - total_payload_written;

    f = pico_socket_frame_alloc(s, get_sock_dev(s),
                                (uint16_t)(space + hdr_offset));
    if (!f) {
      pico_err = PICO_ERR_ENOMEM;
      pico_endpoint_free(ep);
      return -1;
    ...
    if (total_payload_written == 0) {
      ...
      /* Next fragment */
      pico_socket_xmit_next_fragment_setup(
          f, pico_socket_sendto_transport_offset(s), total_payload_written,
          len);
    }

    memcpy(f->payload, (const uint8_t *)buf + total_payload_written,
           f->payload_len);
    transport_flags_update(f, s);
    if (s->proto->push(s->proto, f) > 0) {
...
static uint32_t pico_socket_adapt_mss_to_proto(struct pico_socket *s,
                                               uint32_t mss) {
#ifdef PICO_SUPPORT_IPV6
  if (is_sock_ipv6(s))
...
static int pico_socket_xmit(struct pico_socket *s, const void *buf,
                            const int len, void *src,
                            struct pico_remote_endpoint *ep,
                            struct pico_msginfo *msginfo) {
  int space = pico_socket_xmit_avail_space(s);
  ...
  if ((PROTO(s) == PICO_PROTO_UDP) && (len > space)) {
    total_payload_written =
        pico_socket_xmit_fragments(s, buf, len, src, ep, msginfo);
    /* Implies ep discarding */
    return total_payload_written;
  }
  ...
  while (total_payload_written < len) {
    ...
    w = pico_socket_xmit_one(
        s, (const void *)((const uint8_t *)buf + total_payload_written),
        chunk_len, src, ep, msginfo);
    if (w <= 0) {
      break;
    }
...
int MOCKABLE pico_socket_sendto_extended(struct pico_socket *s, const void *buf,
                                         const int len, void *dst,
  ...
  if (!src) {
#ifdef PICO_SUPPORT_IPV6
    if ((s->net->proto_number == PICO_PROTO_IPV6) && msginfo && msginfo->dev &&
  ...
  return pico_socket_xmit(s, buf, len, src, remote_endpoint,
                          msginfo); /* Implies discarding the endpoint */
}

int MOCKABLE pico_socket_sendto(struct pico_socket *s, const void *buf,
                                const int len, void *dst,
                                uint16_t remote_port) {
...
int pico_socket_recvfrom_extended(struct pico_socket *s, void *buf, int len,
  ...
  if (PROTO(s) == PICO_PROTO_TCP) {
    ...
    if ((s->state & PICO_SOCKET_STATE_SHUT_REMOTE) &&
        pico_tcp_queue_in_is_empty(s)) {
      pico_err = PICO_ERR_ESHUTDOWN;
      return -1;
    } else {
      /* dbg("socket tcp recv\n"); */
...
int MOCKABLE pico_socket_recvfrom(struct pico_socket *s, void *buf, int len,
                                  void *orig, uint16_t *remote_port) {
...
int MOCKABLE pico_socket_bind(struct pico_socket *s, void *local_addr,
                              uint16_t *port) {
...
int pico_socket_connect_no_init(struct pico_socket *s, const void *remote_addr,
                                uint16_t remote_port) {
  int ret = -1;
  pico_err = PICO_ERR_EPROTONOSUPPORT;
  ...
  if (PROTO(s) == PICO_PROTO_TCP) {
    ...
    /*pico_socket_alter_state(s, PICO_SOCKET_STATE_CONNECTED |
     * PICO_SOCKET_STATE_TCP_SYN_SENT, PICO_SOCKET_STATE_CLOSED, 0);*/
    /*pico_err = PICO_ERR_NOERR;*/
    /*ret = 0;*/
    /*} else {*/
    /*pico_err = PICO_ERR_EHOSTUNREACH;*/
...
int pico_socket_connect(struct pico_socket *s, const void *remote_addr,
                        uint16_t remote_port) {
  int ret = -1;
  pico_err = PICO_ERR_EPROTONOSUPPORT;
  if (!s || remote_addr == NULL || remote_port == 0) {
...
int pico_socket_listen(struct pico_socket *s, int backlog) {
  ...
  if (PROTO(s) == PICO_PROTO_TCP)
    pico_socket_alter_state(s, PICO_SOCKET_STATE_TCP_SYN_SENT, 0,
                            PICO_SOCKET_STATE_TCP_LISTEN);
...
struct pico_socket *pico_socket_accept(struct pico_socket *s, void *orig,
  ...
  if (TCPSTATE(s) == PICO_SOCKET_STATE_TCP_LISTEN) {
    ...
    if (sp) {
      ...
      pico_tree_foreach(index, &sp->socks) {
        found = index->keyValue;
        if ((s == found->parent) && ((found->state & PICO_SOCKET_STATE_TCP) ==
                                     PICO_SOCKET_STATE_TCP_ESTABLISHED)) {
          found->parent = NULL;
          pico_err = PICO_ERR_NOERR;
#ifdef PICO_SUPPORT_IPV6
          if (is_sock_ipv6(s)) socklen = sizeof(struct pico_ip6);
...
int MOCKABLE pico_socket_setoption(struct pico_socket *s, int option,
                                   void *value) {
  if (s == NULL) {
    pico_err = PICO_ERR_EINVAL;
    return -1;
  ...
  if (PROTO(s) == PICO_PROTO_TCP) return pico_setsockopt_tcp(s, option, value);

  if (PROTO(s) == PICO_PROTO_UDP) return pico_setsockopt_udp(s, option, value);
...
int pico_socket_getoption(struct pico_socket *s, int option, void *value) {
  ...
  if (PROTO(s) == PICO_PROTO_TCP) return pico_getsockopt_tcp(s, option, value);

  if (PROTO(s) == PICO_PROTO_UDP) return pico_getsockopt_udp(s, option, value);
...
int pico_socket_shutdown(struct pico_socket *s, int mode) {
  ...
  if (PROTO(s) == PICO_PROTO_UDP) {
    if ((mode & PICO_SHUT_RDWR) == PICO_SHUT_RDWR)
      pico_socket_alter_state(s, PICO_SOCKET_STATE_CLOSED,
                              PICO_SOCKET_STATE_CLOSING |
                                  PICO_SOCKET_STATE_BOUND |
  ...
  if (PROTO(s) == PICO_PROTO_TCP) {
    if ((mode & PICO_SHUT_RDWR) == PICO_SHUT_RDWR) {
      pico_socket_alter_state(
          s, PICO_SOCKET_STATE_SHUT_LOCAL | PICO_SOCKET_STATE_SHUT_REMOTE, 0,
          0);
...
#ifdef PICO_SUPPORT_TCP
  if (PROTO(s) == PICO_PROTO_TCP) {
    if (pico_tcp_check_listen_close(s) == 0) return 0;
  }
...
#ifdef PICO_SUPPORT_IPV4
        if (IS_IPV4(f)) checksum_invalid = short_be(pico_udp_checksum_ipv4(f));

#endif
#ifdef PICO_SUPPORT_IPV6
        if (IS_IPV6(f)) checksum_invalid = short_be(pico_udp_checksum_ipv6(f));
...
int pico_transport_process_in(struct pico_protocol *self,
                              struct pico_frame *f) {
  ...
  if ((hdr) && (pico_socket_deliver(self, f, hdr->dport) == 0)) return ret;
...
static int check_socket_sanity(struct pico_socket *s) {
  ...
  if (TCP_STATE(s) == PICO_SOCKET_STATE_TCP_SYN_RECV) {
    if ((PICO_TIME_MS() - s->timestamp) >= PICO_SOCKET_BOUND_TIMEOUT) return -1;
  }
  ...
  while (loop_score > SL_LOOP_MIN && sp_udp != NULL) {
    ...
    pico_tree_foreach(index, &sp_udp->socks) {
      ...
      while (f && (loop_score > 0)) {
        ...
        loop_score -= 1;
        if (loop_score > 0) /* only dequeue if there is still loop_score,
                               otherwise f might get lost */
          f = pico_dequeue(&s->q_out);
      }
    }
...
static int pico_sockets_loop_tcp(int loop_score) {
  ...
  while (loop_score > SL_LOOP_MIN && sp_tcp != NULL) {
    ...
    if (!index_tcp || (index && index->keyValue)) break;
...
int pico_count_sockets(uint8_t proto) {
  ...
  if ((proto == 0) || (proto == PICO_PROTO_TCP)) {
    pico_tree_foreach(idx_sp, &TCPTable) {
      sp = idx_sp->keyValue;
      if (sp) {
        pico_tree_foreach(idx_s, &sp->socks) count++;
      }
    }
  }
...
#ifdef PICO_SUPPORT_IPV6
  if (is_sock_ipv6(s)) f = pico_proto_ipv6.alloc(&pico_proto_ipv6, dev, len);

#endif

#ifdef PICO_SUPPORT_IPV4
  if (is_sock_ipv4(s)) f = pico_proto_ipv4.alloc(&pico_proto_ipv4, dev, len);
