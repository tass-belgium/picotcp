...
If the source can not be found, \texttt{NULL} is returned.
...
On error, -1 is returned if the address is already assigned or no memory could be allocated.
% SET PICO_ERR
...
\subsubsection*{Return value}
On success, this call returns 0.
On error, -1 is returned if the local device interface could not be found. 
...
On error, \texttt{NULL} is returned if the local device could not be found. 
...
\subsubsection*{Return value}
On success, this call returns 0. On error, -1 is returned if the route could not be found.
...
%\subsubsection*{Errors}  ERROR -> pico_err ???
