...
\begin{longtable}{ | l | p{2cm} | p{2cm} | p{5cm} | }
\hline
{\bf Argument} &
{\bf Possible values} &
{\bf Default value} &
...
0,1&
1&
When enabled (=1), the resulting library will contain debug symbols. The size of the library will be much larger than the production build, but it will be possible to run the stack into a debugger to inspect its behaviour. When the option is disabled (=0), the library will be optimized for size in flash, resulting in a smaller binary to be used in production. \\ \hline

PREFIX&
any valid path&
./build&
The target directory where the library and all the objects will be placed after the compilation. \\ \hline
...
0,1&
1&
Enables the support for Transmission Control Protocol by allowing the usage of stream sockets.
\\ \hline

UDP&
0,1&
1&
Enables the support for User Datagram Protocol by allowing the usage of datagram sockets.
\\ \hline
...
0,1&
1&
Enables the support for basic IP networking functionalities. At least one network protocol is required for most of the features to work, as all types of sockets depend on the networking layer.
\\ \hline
...
0,1&
1&
When activated, the ping API will be available to test whether the hosts on the network are reachable. Requires ICMP4 support.
\\ \hline
...
0,1&
1&
Enables the support for IPV6 networking functionalities. At least one network protocol is required for most of the features to work, as all types of sockets depend on the networking layer.
\\ \hline
...
0,1&
1&
When activated, it will be possible to get the IP address for network devices automatically, when a DHCP server is present on the network.
\\ \hline

DHCP$\_$SERVER&
0,1&
1&
If activated, it will be possible to run a small DHCP server to provide addresses for automatic configuration to the other hosts in the network.
\\ \hline
