...
will be possible to run the stack into a debugger to inspect its behavior.
...
\end{verbatim} returns current time expressed in seconds

\item
\begin{verbatim}
   static inline unsigned long PICO_TIME_MS(void)  
\end{verbatim} returns current time expressed in milliseconds

\item
\begin{verbatim}
   static inline void PICO_IDLE(void) 
\end{verbatim} sleep between two consecutive iterations inside 
      the main protocol loop (e.g. to yield the CPU to 
      some other functionality on the sytem)
\end{itemize}

As an alternative to defining the time-keeping procedure in the asynchronous functions \texttt{PICO$\_$TIME()} and
\texttt{PICO$\_$TIME$\_$MS()}, an interrupt handler linked to a fixed
interval time source, increasing the
...
If all the requirements above are satisfied, PicoTCP expects those
functions to be mapped to existing code in the BSP of the architecture.
This can be easily made by the means of a new architecture-specific header
file under the include/arch subdirectory.
Since all the functions above must be already implemented outside the
PicoTCP tree, the library will have to be linked to the system support
library, either during compilation or at a second time when the resulting
...
the \texttt{pico$\_$tick} variable every millisecond, one possible architecture-specific 
...
this example case). One last step is to create a block in the main PicoTCP 
makefile that also sets the needed compiler flags to produce objects compatible 
and/or optimized for the foobar architecture, besides of course defining the 
keyword preprocessor macro so the correct arch-specific header is included:
