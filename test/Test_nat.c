...
//  struct pico_frame *f= pico_frame_alloc(200);
  ...
  if(!ipv4_hdr){
    ...
    printf("hdr is pointer to a buffer\n");
  }
  ...
  if(!tcp_hdr){
    ...
    printf("tcp hdr is pointer to a buffer\n");
  }

//  printf("trans_hdr->sport before = %d\n",trans_hdr->sport);

  printf("----------BEFORE NAT::");
  printf("netw_hdr->dst = %x\n",ipv4_hdr->dst.addr);
  printf("netw_hdr->src = %x\n",ipv4_hdr->src.addr);
//  printf("tcp_hdr->dport = %u\n",tcp_hdr->len);
//  printf("tcp_hdr->sport = %u\n",tcp_hdr->trans.dport);
...
//  printf("ipv4_hdr->proto before = %d\n",ipv4_hdr->proto);
