/*********************************************************************
 ...
 reserved. See COPYING, LICENSE.GPLv2 and LICENSE.GPLv3 for usage. Do not
 redistribute without a written permission by the Copyright holders.
 *********************************************************************/
...
static inline void *pico_zalloc(size_t x) {
  ...
  *ptr = (uint32_t)x; /* store size of alloc */
  ...
  if (cur_mem > max_mem) {
    max_mem = cur_mem;
    if ((MEM_LIMIT > 0) && (max_mem > MEM_LIMIT)) abort();
...
static inline void pico_free(void *x) {
  uint32_t *ptr = (uint32_t *)(((uint8_t *)x) -
                               sizeof(uint32_t)); /* fetch size of the alloc */
  cur_mem -= *ptr;
  free(ptr);
}
