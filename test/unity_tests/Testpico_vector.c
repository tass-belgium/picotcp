...
void test_pico_vector_init(void)
{
    ...
    TEST_ASSERT_EQUAL(vector.type_size, sizeof(struct dummy));
...
void test_pico_vector_grow(void)
{
    ...
    struct dummy d = {42,44,666};
    void* newmem;
    pico_mem_zalloc_ExpectAndReturn(capacity*sizeof(struct dummy), memory);
    TEST_ASSERT_EQUAL_PTR(pico_vector_init(&vector, capacity, sizeof(struct dummy)), memory);
    ...
    newmem = malloc(sizeof(d)*capacity*2);
    pico_mem_zalloc_ExpectAndReturn(capacity*2*sizeof(struct dummy), newmem);
    pico_mem_free_Expect(memory);
...
void test_pico_vector_destroy(void)
{
    ...
    pico_mem_free_Expect(vector.data);
...
void test_pico_iterator(void)
{
    ...
    pico_mem_zalloc_ExpectAndReturn(sizeof(actualIt), &actualIt);
