...
static int pico_igmp_analyse_packet(struct pico_frame *f, struct igmp_packet_params *params)
{
  struct pico_igmp_hdr *hdr = (struct pico_igmp_hdr *) f->transport_hdr;
  switch (hdr->type) {
    case PICO_IGMP_TYPE_MEM_QUERY:
       params->event = PICO_IGMP_EVENT_QUERY_RECV;
       break;
    case PICO_IGMP_TYPE_V1_MEM_REPORT:
       params->event = PICO_IGMP_EVENT_REPORT_RECV;
       break;
...
{
  struct pico_igmp_hdr *igmp_hdr = (struct pico_igmp_hdr *) f->transport_hdr;
  ...
  if (header_checksum == short_be(pico_checksum(igmp_hdr, sizeof(struct pico_igmp_hdr)))) {
...
static int pico_igmp_checksum(struct pico_frame *f)
{
  ...
  igmp_hdr->crc = 0;
  igmp_hdr->crc = short_be(pico_checksum(igmp_hdr, sizeof(struct pico_igmp_hdr)));
  return 0;
}
...
int pico_igmp_join_group(struct pico_ip4 *group_address, struct pico_ipv4_link *link) {
  ...
  params.event = PICO_IGMP_EVENT_JOIN_GROUP ;
...
int pico_igmp_leave_group(struct pico_ip4 *group_address, struct pico_ipv4_link *link) {
  ...
  params.event = PICO_IGMP_EVENT_LEAVE_GROUP ;
  params.group_address = *group_address;
  params.src_interface = link->address;
...
{
  struct mgroup_info *info = pico_igmp_find_mgroup(&(params->group_address));
...
{
  struct mgroup_info *info = pico_igmp_find_mgroup(group_address);
...
static int send_membership_report(struct pico_frame *f)
{
  ...
  group_address.addr = igmp_hdr->group_address;
  dst.addr = igmp_hdr->group_address;
...
static int send_leave(struct pico_frame *f)
{
  ...
  group_address.addr = igmp_hdr->group_address;
  dst.addr = PICO_IGMP_ALL_ROUTER_GROUP;
...
{
  uint8_t ret = 0;
  struct pico_igmp_hdr *igmp_hdr = NULL;

  *f = pico_proto_ipv4.alloc(&pico_proto_ipv4, IP_OPTION_ROUTER_ALERT_LEN + sizeof(struct pico_igmp_hdr));
  ...
  // Fill The IGMP_HDR
  igmp_hdr = (struct pico_igmp_hdr *) (*f)->transport_hdr;

  igmp_hdr->type = type;
  igmp_hdr->max_resp_time = PICO_IGMP_DEFAULT_MAX_RESPONSE_TIME;
  igmp_hdr->group_address = mcast_addr->addr;
...
static void generate_event_timer_expired(long unsigned int empty, void *data)
{
  ...
  params.event = PICO_IGMP_EVENT_TIMER_EXPIRED;
  params.group_address.addr = igmp_hdr->group_address;
...
static int stslifs(struct igmp_packet_params *params)
{
  ...
  ret |= stop_timer(&(params->group_address));
  if (PICO_IGMP_HOST_LAST == info->Last_Host_flag) {
    ret |= create_igmp_frame(&f, params->src_interface, &(params->group_address), PICO_IGMP_TYPE_LEAVE_GROUP);
    ret |= send_leave(f);
  }
...
static int srsfst(struct igmp_packet_params *params)
{
  ...
  info->membership_state = PICO_IGMP_STATES_NON_MEMBER;
  info->Last_Host_flag = PICO_IGMP_HOST_LAST;
  info->active_timer_starttime = NO_ACTIVE_TIMER;
  pico_tree_insert(&KEYTable,info);

  ret |= create_igmp_frame(&f, params->src_interface, &(params->group_address), PICO_IGMP_TYPE_V2_MEM_REPORT);
  ...
  info->delay = (pico_rand() % (PICO_IGMP_UNSOLICITED_REPORT_INTERVAL * 100)); 
  params->f = f;
  ret |= start_timer(params, info->delay);

  if(0 == ret) {
    struct mgroup_info *info = pico_igmp_find_mgroup(&(params->group_address));
    info->membership_state = PICO_IGMP_STATES_DELAYING_MEMBER;
...
static int slifs(struct igmp_packet_params *params)
{
  ...
  info = pico_igmp_find_mgroup(&(params->group_address));
  if (PICO_IGMP_HOST_LAST == info->Last_Host_flag) {
    ret |= create_igmp_frame(&f, params->src_interface, &(params->group_address), PICO_IGMP_TYPE_LEAVE_GROUP);
    send_leave(f);
  }
...
static int st(struct igmp_packet_params *params)
{
  ...
  ret |= create_igmp_frame(&(params->f), info->src_interface, &(params->group_address), PICO_IGMP_TYPE_V2_MEM_REPORT);
...
static int stcl(struct igmp_packet_params *params)
{
  ...
  ret |= stop_timer(&(params->group_address));
  info->Last_Host_flag = PICO_IGMP_HOST_NOT_LAST;

  if (0 == ret) {
    info->membership_state = PICO_IGMP_STATES_IDLE_MEMBER;
...
static int rtimrtct(struct igmp_packet_params *params)
{
  ...
  if (((unsigned long)(params->max_resp_time * 100)) < current_time_left) {
    ret |= create_igmp_frame(&(params->f), params->src_interface, &(params->group_address), PICO_IGMP_TYPE_V2_MEM_REPORT);
...
static int pico_igmp_process_event(struct igmp_packet_params *params)
{
  ...
  if (NULL == info) {
    if (params->event == PICO_IGMP_EVENT_QUERY_RECV) { /* general query */
      pico_tree_foreach(index,&KEYTable) {
        ...
        igmp_dbg("IGMP: for each group_address = %08X | state = %u\n", params->group_address.addr, info->membership_state);
      ...
      ret |= host_membership_diagram_table[PICO_IGMP_STATES_NON_MEMBER][params->event](params);
    }
  } else {
