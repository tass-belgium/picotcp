 ...
 *			ession pointer into account but not the second byte, which
 *acts like a trailing zero-byte
 * ****************************************************************************/
...
int pico_dns_mirror_addr(char *ip) {
  ...
  if (pico_string_to_ipv4(ip, &addr) < 0) return -1;
...
int pico_dns_question_delete(void **question) {
  ...
  if ((*q)->qname) PICO_FREE(((*q)->qname));

  if ((*q)->qsuffix) PICO_FREE((*q)->qsuffix);
 ...
 *				   format. When reverse resolving, only the IP, either
 *IPV4 or IPV6, should be given in string format. f.e. => for IPv4:
 *"192.168.2.1"
 *						=> for IPv6:
 ...
 *					   to. When function returns, this will point to
 *location right after the flat copied DNS Resource Record.
 *  @return Returns 0 on success, something else on failure.
 * ****************************************************************************/
...
int pico_dns_record_delete(void **record) {
  ...
  if ((*rr)->rname) PICO_FREE((*rr)->rname);

  if ((*rr)->rsuffix) PICO_FREE((*rr)->rsuffix);

  if ((*rr)->rdata) PICO_FREE((*rr)->rdata);
 ...
 *				   data should contain a DNS name, the name in the
 *databuffer needs to be in URL-format.
 *  @param datalen The exact length in bytes of the _rdata-buffer. If data of
 *				   record should contain a DNS name, datalen needs to
 *be pico_dns_strlen(_rdata).
 *  @param rtype   DNS type of the resource record to be
 ...
 * ****************************************************************************/
struct pico_dns_record *pico_dns_record_create(const char *url, void *_rdata,
...
int pico_dns_rdata_cmp(uint8_t *a, uint8_t *b, uint16_t rdlength_a,
  ...
  slen = rdlength_a;
  if (rdlength_b < slen) slen = rdlength_b;
...
int pico_dns_question_cmp(void *qa, void *qb) {
  ...
  if ((dif = (int)((int)at - (int)bt))) return dif;
...
int pico_dns_record_cmp(void *ra, void *rb) {
  ...
  if ((dif = pico_dns_record_cmp_name_type(a, b))) return dif;
 ...
 *					 Make sure you clear out (set to 0) this param
 *before you call this function because it doesn't happen inside and each size
...
int pico_dns_qtree_find_name(struct pico_tree *qtree, const char *name) {
  ...
  pico_tree_foreach(node, qtree) {
    question = (struct pico_dns_question *)node->keyValue;
    if ((question) && (strcasecmp(question->qname, name) == 0)) return 1;
  }
...
static uint8_t *pico_dns_packet_compress_find_ptr(uint8_t *name, uint8_t *data,
  ...
  /* Check params */
  if (!name || !data || !len) return NULL;

  if ((name < data) || (name > (data + len))) return NULL;
...
static int pico_dns_compress_record_sections(uint16_t qdcount, uint16_t count,
  ...
  for (i = 0; i < count; i++) {
    if (qdcount || i) pico_dns_packet_compress_name(_iterator, buf, len);
