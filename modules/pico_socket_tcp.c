...
int pico_getsockopt_tcp(struct pico_socket *s, int option, void *value) {
  if (sockopt_validate_args(s, value) < 0) return -1;
...
static void tcp_set_nagle_option(struct pico_socket *s, void *value) {
  int *val = (int *)value;
  if (*val > 0) {
    tcp_dbg("setsockopt: Nagle algorithm disabled.\n");
    PICO_SOCKET_SETOPT_EN(s, PICO_SOCKET_OPT_TCPNODELAY);
  } else {
    tcp_dbg("setsockopt: Nagle algorithm enabled.\n");
...
void pico_socket_tcp_cleanup(struct pico_socket *sock) {
  ...
  if (is_sock_tcp(sock)) pico_tcp_cleanup_queues(sock);
...
#ifdef PICO_SUPPORT_TCP
  if (s->parent) s->parent->number_of_pending_conn--;

#endif
}

static struct pico_socket *socket_tcp_deliver_ipv4(struct pico_socket *s,
                                                   struct pico_frame *f) {
  ...
  if ((s->remote_port == tr->sport) && /* remote port check */
      ...
      ((s_local.addr == PICO_IPV4_INADDR_ANY) ||
       (s_local.addr ==
        p_dst.addr))) { /* Either local socket is ANY, or matches dst */
    found = s;
    return found;
...
static struct pico_socket *socket_tcp_deliver_ipv6(struct pico_socket *s,
                                                   struct pico_frame *f) {
  struct pico_socket *found = NULL;
#ifdef PICO_SUPPORT_IPV6
  struct pico_trans *tr = (struct pico_trans *)f->transport_hdr;
  struct pico_ip6 s_local = {{0}}, s_remote = {{0}}, p_src = {{0}},
                  p_dst = {{0}};
  ...
  if ((s->remote_port == tr->sport) &&
      ...
      ((!memcmp(s_local.addr, PICO_IP6_ANY, PICO_SIZE_IP6)) ||
       (!memcmp(s_local.addr, p_dst.addr, PICO_SIZE_IP6)))) {
    found = s;
    return found;
  } else if ((s->remote_port == 0) && /* not connected... listening */
             ((!memcmp(s_local.addr, PICO_IP6_ANY, PICO_SIZE_IP6)) ||
...
int pico_socket_tcp_deliver(struct pico_sockport *sp, struct pico_frame *f) {
  ...
  pico_tree_foreach_safe(index, &sp->socks, _tmp) {
    ...
    if (found) {
      tcp_dbg("FOUND\n");
      target = found;
      if (found->remote_port != 0) /* only break if it's connected */
        break;
    }
  } /* FOREACH */
...
#ifdef PICO_SUPPORT_TCP
  if (is_sock_tcp(s)) pico_tcp_flags_update(f, s);
