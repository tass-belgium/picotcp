...
#define SEQN(f) \
  ((f) ? (long_be(((struct pico_tcp_hdr *)((f)->transport_hdr))->seq)) : 0)
#define ACKN(f) \
...
#define TCP_INPUT_OVERHEAD \
  (sizeof(struct tcp_input_segment) + sizeof(struct pico_tree_node))
...
static void *next_segment(struct pico_tcp_queue *tq, void *cur) {
  ...
  if (IS_INPUT_QUEUE(tq)) {
    return peek_segment(tq, ((struct tcp_input_segment *)cur)->seq +
                                ((struct tcp_input_segment *)cur)->payload_len);
  } else {
    return peek_segment(tq, SEQN((struct pico_frame *)cur) +
                                ((struct pico_frame *)cur)->payload_len);
  }
}
...
static int32_t do_enqueue_segment(struct pico_tcp_queue *tq, void *f,
                                  uint16_t payload_len) {
  int32_t ret = -1;
...
static void pico_discard_segment(struct pico_tcp_queue *tq, void *f) {
  ...
  uint16_t payload_len = (uint16_t)(
      (IS_INPUT_QUEUE(tq)) ? (((struct tcp_input_segment *)f)->payload_len)
...
struct pico_socket_tcp {
  ...
  /* Tree/queues */
  struct pico_tcp_queue tcpq_in; /* updated the input queue to hold input
                                    segments not the full frame. */
  struct pico_tcp_queue tcpq_out;
  struct pico_tcp_queue
      tcpq_hold; /* buffer to hold delayed frames according to Nagle */
...
/* If Nagle enabled, this function can make 1 new segment from smaller segments
 * in hold queue */
...
static int release_until(struct pico_tcp_queue *q, uint32_t seq) {
  ...
  do {
    ...
    if (IS_INPUT_QUEUE(q))
      seq_result =
          pico_seq_compare(((struct tcp_input_segment *)head)->seq +
                               ((struct tcp_input_segment *)head)->payload_len,
                           seq);
    else
      seq_result =
          pico_seq_compare(SEQN((struct pico_frame *)head) +
                               ((struct pico_frame *)head)->payload_len,
                           seq);

    if (seq_result <= 0) {
      head = next_segment(q, cur);
      /* tcp_dbg("Releasing %08x, len: %d\n", SEQN((struct pico_frame *)head),
       * ((struct pico_frame *)head)->payload_len); */
...
static int release_all_until(struct pico_tcp_queue *q, uint32_t seq,
  ...
  *timestamp = 0;
  ...
  pico_tree_foreach_safe(idx, &q->pool, temp) {
    ...
    if (IS_INPUT_QUEUE(q))
      seq_result =
          pico_seq_compare(((struct tcp_input_segment *)f)->seq +
                               ((struct tcp_input_segment *)f)->payload_len,
                           seq);
    else
      seq_result = pico_seq_compare(
          SEQN((struct pico_frame *)f) + ((struct pico_frame *)f)->payload_len,
          seq);
...
uint16_t pico_tcp_checksum_ipv4(struct pico_frame *f) {
  ...
  return pico_dualbuffer_checksum(&pseudo, sizeof(struct pico_ipv4_pseudo_hdr),
                                  tcp_hdr, f->transport_len);
}
...
uint16_t pico_tcp_checksum_ipv6(struct pico_frame *f) {
  ...
  return pico_dualbuffer_checksum(&pseudo, sizeof(struct pico_ipv6_pseudo_hdr),
                                  tcp_hdr, f->transport_len);
}
#endif
...
#ifdef PICO_SUPPORT_IPV4
  if (checksum_is_ipv4(f)) return pico_tcp_checksum_ipv4(f);

#endif

#ifdef PICO_SUPPORT_IPV6
  if (checksum_is_ipv6(f)) return pico_tcp_checksum_ipv6(f);
...
static int pico_tcp_process_out(struct pico_protocol *self,
                                struct pico_frame *f) {
  ...
  if (f->payload_len > 0) {
    ...
    /* hdr->seq = long_be(t->snd_nxt);   / * XXX disabled this to not to mess
     * with seq nrs of ACKs anymore * / */
  } else {
...
static inline void tcp_add_sack_option(struct pico_socket_tcp *ts,
                                       struct pico_frame *f, uint16_t flags,
                                       uint32_t *ii) {
    ...
    if (ts->sack_ok && ts->sacks) {
      ...
      while (ts->sacks) {
        ...
        *ii += (2 * (uint32_t)sizeof(uint32_t));
        f->start[len_off] =
            (uint8_t)(f->start[len_off] + (2 * sizeof(uint32_t)));
        PICO_FREE(sb);
      }
    }
  }
...
static void tcp_add_options(struct pico_socket_tcp *ts, struct pico_frame *f,
                            uint16_t flags, uint16_t optsiz) {
  ...
  if (i < optsiz) f->start[optsiz - 1] = PICO_TCP_OPTION_END;
}
...
static void tcp_add_options_frame(struct pico_socket_tcp *ts,
                                  struct pico_frame *f) {
  uint32_t tsval = long_be((uint32_t)TCP_TIME);
  uint32_t tsecr = long_be(ts->ts_nxt);
  uint32_t i = 0;
...
static inline void tcp_set_space_check_winupdate(struct pico_socket_tcp *t,
                                                 int32_t space,
                                                 uint32_t shift) {
  if (((uint32_t)space != t->wnd) || (shift != t->wnd_scale) ||
      ((space - t->wnd) > (int32_t)((uint32_t)space >> 2u))) {
...
static uint16_t tcp_options_size(struct pico_socket_tcp *t, uint16_t flags) {
  ...
  if (flags & PICO_TCP_SYN) { /* Full options */
    size = PICO_TCPOPTLEN_MSS + PICO_TCP_OPTION_SACK_OK + PICO_TCPOPTLEN_WS +
           PICO_TCPOPTLEN_TIMESTAMP;
  } else {
    /* Always update window scale. */
    size = (uint16_t)(size + PICO_TCPOPTLEN_WS);

    if (t->ts_ok) size = (uint16_t)(size + PICO_TCPOPTLEN_TIMESTAMP);
...
uint16_t pico_tcp_overhead(struct pico_socket *s) {
  ...
  return (uint16_t)(
      PICO_SIZE_TCPHDR +
      tcp_options_size((struct pico_socket_tcp *)s,
                       (uint16_t)0)); /* hdr + Options size for data pkt */
}

static inline int tcp_sack_marker(struct pico_frame *f, uint32_t start,
                                  uint32_t end, uint16_t *count) {
  int cmp;
...
static void tcp_process_sack(struct pico_socket_tcp *t, uint32_t start,
                             uint32_t end) {
  struct pico_frame *f;
  ...
  pico_tree_foreach_safe(index, &t->tcpq_out.pool, temp) {
    f = index->keyValue;
    if (tcp_sack_marker(f, start, end, &count) == 0) goto done;
  }
...
inline static void tcp_add_header(struct pico_socket_tcp *t,
                                  struct pico_frame *f) {
  struct pico_tcp_hdr *hdr = (struct pico_tcp_hdr *)f->transport_hdr;
  f->timestamp = TCP_TIME;
  tcp_add_options(t, f, 0,
                  (uint16_t)(f->transport_len - f->payload_len -
                             (uint16_t)PICO_SIZE_TCPHDR));
...
static inline void tcp_parse_option_ws(struct pico_socket_tcp *t, uint8_t len,
                                       uint8_t *opt, uint32_t *idx) {
  if (tcpopt_len_check(idx, len, PICO_TCPOPTLEN_WS) < 0) return;

  t->recv_wnd_scale = opt[(*idx)++];
  tcp_dbg_options("TCP Window scale: received %d\n", t->recv_wnd_scale);
}

static inline void tcp_parse_option_sack_ok(struct pico_socket_tcp *t,
                                            struct pico_frame *f, uint8_t len,
                                            uint32_t *idx) {
  if (tcpopt_len_check(idx, len, PICO_TCPOPTLEN_SACK_OK) < 0) return;

  if (((struct pico_tcp_hdr *)(f->transport_hdr))->flags & PICO_TCP_SYN)
    t->sack_ok = 1;
}

static inline void tcp_parse_option_mss(struct pico_socket_tcp *t, uint8_t len,
                                        uint8_t *opt, uint32_t *idx) {
  uint16_t mss;
  if (tcpopt_len_check(idx, len, PICO_TCPOPTLEN_MSS) < 0) return;

  t->mss_ok = 1;
  mss = short_from(opt + *idx);
  *idx += (uint32_t)sizeof(uint16_t);
  if (t->mss > short_be(mss)) t->mss = short_be(mss);
}

static inline void tcp_parse_option_timestamp(struct pico_socket_tcp *t,
                                              struct pico_frame *f, uint8_t len,
                                              uint8_t *opt, uint32_t *idx) {
  uint32_t tsval, tsecr;
  if (tcpopt_len_check(idx, len, PICO_TCPOPTLEN_TIMESTAMP) < 0) return;
...
static void tcp_parse_options(struct pico_frame *f) {
  ...
  while (i < (f->transport_len - PICO_SIZE_TCPHDR)) {
    ...
    if (f->payload && ((opt + i) > f->payload)) break;
...
static inline int tcp_send_try_enqueue(struct pico_socket_tcp *ts,
                                       struct pico_frame *f) {
  struct pico_tcp_hdr *hdr = (struct pico_tcp_hdr *)f->transport_hdr;
  ...
  if ((pico_enqueue(&tcp_out, cpy) > 0)) {
    if (f->payload_len > 0) {
      ts->in_flight++;
      ts->snd_nxt +=
          f->payload_len; /* update next pointer here to prevent sending same
                             segment twice when called twice in same tick */
    }

    tcp_dbg(
        "DBG> [tcp output] state: %02x --> local port:%u remote port: %u seq: "
        "%08x ack: %08x flags: %02x = t_len: %u, hdr: %u payload: %d\n",
        TCPSTATE(&ts->sock) >> 8, short_be(hdr->trans.sport),
        short_be(hdr->trans.dport), SEQN(f), ACKN(f), hdr->flags,
        f->transport_len, (hdr->len & 0xf0) >> 2, f->payload_len);
  } else {
...
static int tcp_send(struct pico_socket_tcp *ts, struct pico_frame *f) {
  ...
  if (!hdr->seq) hdr->seq = long_be(ts->snd_nxt);
...
static void sock_stats(uint32_t when, void *arg) {
  ...
  tcp_dbg(
      "STATISTIC> [%lu] socket state: %02x --> local port:%d remote port: %d "
      "queue size: %d snd_una: %08x snd_nxt: %08x cwnd: %d\n",
      when, t->sock.state, short_be(t->sock.local_port),
...
static void pico_tcp_keepalive(pico_time now, void *arg) {
  ...
  if (((t->sock.state & PICO_SOCKET_STATE_TCP) ==
    ...
    if (t->ka_time < (now - t->ack_timestamp)) {
      ...
      if (((t->ka_retries_count * (pico_time)t->ka_intvl) + t->ka_time) <
          (now - t->ack_timestamp)) {
        /* Next probe */
        tcp_send_probe(t);
        t->ka_retries_count++;
  ...
  if (!t->keepalive_tmr) {
    ...
    if (t->sock.wakeup) t->sock.wakeup(PICO_SOCK_EV_ERR, &t->sock);
  }
}

static inline void rto_set(struct pico_socket_tcp *t, uint32_t rto) {
  if (rto < PICO_TCP_RTO_MIN) rto = PICO_TCP_RTO_MIN;

  if (rto > PICO_TCP_RTO_MAX) rto = PICO_TCP_RTO_MAX;
...
static inline uint32_t tcp_read_in_frame_len(struct tcp_input_segment *f,
                                             int32_t in_frame_off,
                                             uint32_t tot_rd_len,
                                             uint32_t read_op_len) {
  ...
  // this is tricking the caller to think that the payload's maximum size is the
  // total len requested in the optional argument this is useful because it
  // makes sure that in_frame_len represents the actual size of the payload that
  // we are ready to process, so after we are done, we can drop that chunk from
  // the queue.
...
static inline uint32_t tcp_read_in_frame_len_readline(
    struct tcp_input_segment *f, int32_t in_frame_off) {
  uint32_t in_frame_len = 0;
  if (in_frame_off > 0) {
    if ((uint32_t)in_frame_off > f->payload_len) {
      tcp_dbg("FATAL TCP ERR: in_frame_off > f->payload_len\n");
...
static inline void tcp_read_check_segment_done(struct pico_socket_tcp *t,
...
uint32_t pico_tcp_read(struct pico_socket *s, void *buf, uint32_t len) {
  ...
  while (tot_rd_len < len) {
    ...
    memcpy((uint8_t *)buf + tot_rd_len, f->payload + in_frame_off,
           in_frame_len);
...
int pico_tcp_initconn(struct pico_socket *s) {
  ...
  if (!ts->snd_nxt) ts->snd_nxt = long_be(pico_paws());
  ...
  ts->ssthresh =
      (uint16_t)((uint16_t)(PICO_DEFAULT_SOCKETQ / ts->mss) -
                 (((uint16_t)(PICO_DEFAULT_SOCKETQ / ts->mss)) >> 3u));
  syn->sock = s;
...
static void tcp_send_empty(struct pico_socket_tcp *t, uint16_t flags,
  ...
  f = t->sock.net->alloc(t->sock.net, NULL,
                         (uint16_t)(PICO_SIZE_TCPHDR + opt_len));
  if (!f) {
    return;
  }
  ...
  if (is_keepalive) hdr->seq = long_be(t->snd_nxt - 1);
...
static int tcp_send_rst(struct pico_socket *s, struct pico_frame *fr) {
  ...
  if (fr &&
      ((s->state & PICO_SOCKET_STATE_TCP) > PICO_SOCKET_STATE_TCP_SYN_RECV)) {
    ...
    if ((t->sock).wakeup) (t->sock).wakeup(PICO_SOCK_EV_FIN, &(t->sock));
...
static inline void tcp_fill_rst_payload(struct pico_frame *fr,
                                        struct pico_frame *f) {
  ...
  if (IS_IPV4(fr)) {
    ...
    ((struct pico_ipv4_hdr *)(f->net_hdr))->dst.addr =
        ((struct pico_ipv4_hdr *)(fr->net_hdr))->src.addr;
    ((struct pico_ipv4_hdr *)(f->net_hdr))->src.addr =
        ((struct pico_ipv4_hdr *)(fr->net_hdr))->dst.addr;
    tcp_dbg("Making IPv4 reset frame...\n");

  } else {
    memcpy(f->net_hdr, fr->net_hdr, sizeof(struct pico_ipv6_hdr));
    ((struct pico_ipv6_hdr *)(f->net_hdr))->dst =
        ((struct pico_ipv6_hdr *)(fr->net_hdr))->src;
    ((struct pico_ipv6_hdr *)(f->net_hdr))->src =
        ((struct pico_ipv6_hdr *)(fr->net_hdr))->dst;
  }

  /* fill in TCP data from original frame */
  ((struct pico_tcp_hdr *)(f->transport_hdr))->trans.dport =
      ((struct pico_tcp_hdr *)(fr->transport_hdr))->trans.sport;
  ((struct pico_tcp_hdr *)(f->transport_hdr))->trans.sport =
      ((struct pico_tcp_hdr *)(fr->transport_hdr))->trans.dport;
}

static inline void tcp_fill_rst_header(struct pico_frame *fr,
                                       struct pico_tcp_hdr *hdr1,
                                       struct pico_frame *f,
                                       struct pico_tcp_hdr *hdr) {
  if (!(hdr1->flags & PICO_TCP_ACK)) hdr->flags |= PICO_TCP_ACK;
  ...
  if (!(hdr1->flags & PICO_TCP_ACK))
    hdr->ack =
        long_be(long_be(((struct pico_tcp_hdr *)(fr->transport_hdr))->seq) +
                fr->payload_len);
...
int pico_tcp_reply_rst(struct pico_frame *fr) {
  ...
  if ((hdr1->flags & PICO_TCP_RST) != 0) return -1;
  ...
  if (0) {
    ...
    pico_ipv4_frame_push(f, &(((struct pico_ipv4_hdr *)(f->net_hdr))->dst),
                         PICO_PROTO_TCP);
#endif
#ifdef PICO_SUPPORT_IPV6
  } else {
    pico_ipv6_frame_push(f, NULL,
                         &(((struct pico_ipv6_hdr *)(f->net_hdr))->dst),
                         PICO_PROTO_TCP, 0);
#endif
  }
...
static int tcp_nosync_rst(struct pico_socket *s, struct pico_frame *fr) {
  ...
  if (((s->state & PICO_SOCKET_STATE_TCP) == PICO_SOCKET_STATE_TCP_LISTEN)) {
    if ((fr->flags & PICO_TCP_RST) != 0) return 0;
...
static inline int tcp_data_in_expected(struct pico_socket_tcp *t,
                                       struct pico_frame *f) {
  struct tcp_input_segment *nxt;
  if (pico_seq_compare(SEQN(f), t->rcv_nxt) ==
      0) { /* Exactly what we expected */
    ...
    tcp_dbg(
        "TCP> lo segment. Uninteresting retransmission. (exp: %x got: %x)\n",
        t->rcv_nxt, SEQN(f));
  }
...
static inline int tcp_data_in_high_segment(struct pico_socket_tcp *t,
                                           struct pico_frame *f) {
  tcp_dbg(
      "TCP> hi segment. Possible packet loss. I'll dupack this. (exp: %x got: "
      "%x)\n",
      t->rcv_nxt, SEQN(f));
  if (t->sack_ok) {
    struct tcp_input_segment *input = segment_from_frame(f);
...
static inline void tcp_data_in_send_ack(struct pico_socket_tcp *t,
                                        struct pico_frame *f) {
  struct pico_tcp_hdr *hdr = (struct pico_tcp_hdr *)f->transport_hdr;
  /* In either case, ack til recv_nxt, unless received data raises a RST flag.
   */
  if (((t->sock.state & PICO_SOCKET_STATE_TCP) !=
       PICO_SOCKET_STATE_TCP_CLOSE_WAIT) &&
...
static int tcp_data_in(struct pico_socket *s, struct pico_frame *f) {
  ...
  struct pico_tcp_hdr *hdr = (struct pico_tcp_hdr *)f->transport_hdr;
  uint16_t payload_len =
      (uint16_t)(f->transport_len - ((hdr->len & 0xf0u) >> 2u));
  int ret = 0;
  (void)hdr;
...
static void tcp_rtt(struct pico_socket_tcp *t, uint32_t rtt) {
  ...
  if (!avg) {
    ...
    /* Finally, assign a new value for the RTO, as specified in the RFC, with
     * K=4 */
    rto_set(t, t->avg_rtt + (t->rttvar << 2));
  }
...
static void tcp_congestion_control(struct pico_socket_tcp *t) {
  if (t->x_mode > PICO_TCP_LOOKAHEAD) return;
  ...
  tcp_dbg("TCP_CWND, %lu, %u, %u, %u\n", TCP_TIME, t->cwnd, t->ssthresh,
          t->in_flight);
}

static void add_retransmission_timer(struct pico_socket_tcp *t,
                                     pico_time next_ts);
...
static inline int tcp_retrans_timeout_check_queue(struct pico_socket_tcp *t) {
  ...
  while (f) {
    ...
    if (t->x_mode == PICO_TCP_WINDOW_FULL) {
      tcp_dbg(
          "TCP BLACKOUT> TIMED OUT (output) frame %08x, len= %d rto=%d Win "
          "full: %d frame flags: %04x\n",
          SEQN(f), f->payload_len, t->rto, t->x_mode == PICO_TCP_WINDOW_FULL,
          f->flags);
      tcp_next_zerowindow_probe(t);
      return -1;
    }

    if (t->x_mode != PICO_TCP_BLACKOUT) tcp_first_timeout(t);
...
static void tcp_retrans_timeout(pico_time val, void *sock) {
  ...
  if (tcp_is_allowed_to_send(t)) {
    if (tcp_retrans_timeout_check_queue(t) < 0) return;
  } else if (t->backoff >= PICO_TCP_MAX_RETRANS &&
             ((t->sock.state & PICO_SOCKET_STATE_TCP) ==
                  PICO_SOCKET_STATE_TCP_ESTABLISHED ||
              (t->sock.state & PICO_SOCKET_STATE_TCP) ==
                  PICO_SOCKET_STATE_TCP_FIN_WAIT1 ||
              (t->sock.state & PICO_SOCKET_STATE_TCP) ==
                  PICO_SOCKET_STATE_TCP_FIN_WAIT2 ||
              (t->sock.state & PICO_SOCKET_STATE_TCP) ==
    ...
    /* the retransmission timer, failed to get an ack for a frame, gives up on
     * the connection */
...
static int tcp_ack(struct pico_socket *s, struct pico_frame *f) {
  ...
  if ((hdr->flags & PICO_TCP_ACK) == 0) return -1;
  ...
  if ((t->x_mode == PICO_TCP_BLACKOUT) ||
      ((t->x_mode == PICO_TCP_WINDOW_FULL) &&
       ((t->recv_wnd << t->recv_wnd_scale) > t->mss))) {
  ...
  if (!una || acked > 0) {
    ...
    tcp_dbg(
        "TCP ACK> FRESH ACK %08x (acked %d) Queue size: %u/%u frames: %u cwnd: "
        "%u in_flight: %u snd_una: %u\n",
        ACKN(f), acked, t->tcpq_out.size, t->tcpq_out.max_size,
        t->tcpq_out.frames, t->cwnd, t->in_flight, SEQN(una));
  ...
  } else if ((t->snd_old_ack ==
              ACKN(f)) && /* We've just seen this ack, and... */
             ((0 == (hdr->flags & (PICO_TCP_PSH | PICO_TCP_SYN))) &&
              (f->payload_len == 0)) && /* This is a pure ack, and... */
             (ACKN(f) != t->snd_nxt))   /* There is something in flight awaiting
                                           to be acked... */
  {
    ...
    if (t->x_mode < PICO_TCP_RECOVER) {
      ...
      if (t->x_mode == PICO_TCP_RECOVER) { /* Switching mode */
        ...
        if (t->ssthresh < 2) t->ssthresh = 2;
      }
    } else if (t->x_mode == PICO_TCP_RECOVER) {
      /* tcp_dbg("TCP RECOVER> DUPACK! snd_una: %08x, snd_nxt: %08x, acked now:
       * %08x\n", SEQN(first_segment(&t->tcpq_out)), t->snd_nxt, ACKN(f)); */
      if (t->in_flight <= t->cwnd) {
        struct pico_frame *nxt = peek_segment(&t->tcpq_out, t->snd_retry);
        if (!nxt) nxt = first_segment(&t->tcpq_out);
        ...
        if (nxt && (pico_seq_compare(SEQN(nxt), t->snd_nxt)) > 0) nxt = NULL;

        if (nxt && (pico_seq_compare(SEQN(nxt),
                                     SEQN((struct pico_frame *)first_segment(
                                         &t->tcpq_out))) >
                    (int)(t->recv_wnd << t->recv_wnd_scale)))
          nxt = NULL;
  ...
  if ((0 == (hdr->flags &
             (PICO_TCP_PSH | PICO_TCP_SYN))) && /* This is a pure ack, and... */
  ...
  if (IS_NAGLE_ENABLED((&(t->sock)))) {
    while (!IS_TCP_HOLDQ_EMPTY(t) &&
      ...
      if (f_new == NULL) break; /* XXX corrupt !!! (or no memory) */

      if (pico_enqueue_segment(&t->tcpq_out, f_new) <= 0) /* handle error */
...
static int tcp_finwaitfin(struct pico_socket *s, struct pico_frame *f) {
  ...
  /* set SHUT_REMOTE */
  s->state |= PICO_SOCKET_STATE_SHUT_REMOTE;
  if (s->wakeup) s->wakeup(PICO_SOCK_EV_CLOSE, s);
...
static int tcp_lastackwait(struct pico_socket *s, struct pico_frame *f) {
  ...
  if (ACKN(f) == t->snd_nxt) {
    ...
    if (s->wakeup) s->wakeup(PICO_SOCK_EV_FIN, s);
...
static int tcp_syn(struct pico_socket *s, struct pico_frame *f) {
  ...
  uint16_t mtu;
  if (s->number_of_pending_conn >= s->max_backlog) return -1;
  ...
  new->ssthresh =
      (uint16_t)((uint16_t)(PICO_DEFAULT_SOCKETQ / new->mss) -
...
static int tcp_synack(struct pico_socket *s, struct pico_frame *f) {
  ...
  if (ACKN(f) == (1u + t->snd_nxt)) {
    ...
    if (s->wakeup) s->wakeup(PICO_SOCK_EV_CONN, s);
...
static int tcp_first_ack(struct pico_socket *s, struct pico_frame *f) {
  ...
  if (t->snd_nxt == ACKN(f)) {
    ...
    if (!s->parent && s->wakeup) { /* If the socket has no parent, -> sending
                                      socket that has a sim_open */
...
static void tcp_attempt_closewait(struct pico_socket *s, struct pico_frame *f) {
  ...
  if (pico_seq_compare(SEQN(f), t->rcv_nxt) == 0) {
    ...
    if (pico_seq_compare(SEQN(f), t->rcv_processed) == 0) {
      if ((s->state & PICO_SOCKET_STATE_TCP) ==
          PICO_SOCKET_STATE_TCP_ESTABLISHED) {
...
static int tcp_closewait(struct pico_socket *s, struct pico_frame *f) {
  ...
  if (f->payload_len > 0) tcp_data_in(s, f);

  if (hdr->flags & PICO_TCP_ACK) tcp_ack(s, f);

  tcp_dbg(
      "called close_wait (%p), in state %08x, f->flags: 0x%02x, hdr->flags: "
      "0x%02x\n",
   ...
   * pico_socket_close() has been called in the socket callback, we don't need
   * to send an ACK here.
   *
   */
  if (((s->state & PICO_SOCKET_STATE_TCP) ==
...
static void tcp_wakeup_pending(struct pico_socket *s, uint16_t ev) {
  struct pico_socket_tcp *t = (struct pico_socket_tcp *)s;
  if ((t->sock).wakeup) (t->sock).wakeup(ev, &(t->sock));
}
...
static int tcp_rst(struct pico_socket *s, struct pico_frame *f) {
  ...
  if ((s->state & PICO_SOCKET_STATE_TCP) == PICO_SOCKET_STATE_TCP_SYN_SENT) {
    ...
    if ((this_seq >= t->rcv_ackd) &&
        (this_seq <=
         ((uint32_t)(short_be(hdr->rwnd) << (t->wnd_scale)) + t->rcv_ackd))) {
      tcp_force_closed(s);
    } else { /* not valid, ignore */
      tcp_dbg("TCP RST> IGNORE\n");
      return 0;
...
static const struct tcp_action_entry tcp_fsm[] = {
    ...
    {PICO_SOCKET_STATE_TCP_LISTEN, &tcp_syn, NULL, NULL, NULL, NULL, NULL,
     NULL},
    {PICO_SOCKET_STATE_TCP_SYN_SENT, NULL, &tcp_synack, NULL, NULL, NULL, NULL,
     &tcp_rst},
    {PICO_SOCKET_STATE_TCP_SYN_RECV, &tcp_synrecv_syn, NULL, &tcp_first_ack,
     &tcp_data_in, NULL, &tcp_closeconn, &tcp_rst},
    {PICO_SOCKET_STATE_TCP_ESTABLISHED, &tcp_halfopencon, &tcp_ack, &tcp_ack,
     &tcp_data_in, &tcp_closewait, &tcp_closewait, &tcp_rst},
    {PICO_SOCKET_STATE_TCP_CLOSE_WAIT, NULL, &tcp_ack, &tcp_ack, &tcp_send_rst,
     &tcp_closewait, &tcp_closewait, &tcp_rst},
    {PICO_SOCKET_STATE_TCP_LAST_ACK, NULL, &tcp_ack, &tcp_lastackwait,
     &tcp_send_rst, &tcp_send_rst, &tcp_send_rst, &tcp_rst},
    {PICO_SOCKET_STATE_TCP_FIN_WAIT1, NULL, &tcp_ack, &tcp_finwaitack,
     &tcp_data_in, &tcp_rcvfin, &tcp_finack, &tcp_rst},
    {PICO_SOCKET_STATE_TCP_FIN_WAIT2, NULL, &tcp_ack, &tcp_ack, &tcp_data_in,
     &tcp_finwaitfin, &tcp_finack, &tcp_rst},
    {PICO_SOCKET_STATE_TCP_CLOSING, NULL, &tcp_ack, &tcp_closing_ack,
     &tcp_send_rst, &tcp_send_rst, &tcp_send_rst, &tcp_rst},
    {PICO_SOCKET_STATE_TCP_TIME_WAIT, NULL, NULL, NULL, &tcp_send_rst, NULL,
     NULL, NULL}};
...
static uint8_t invalid_flags(struct pico_socket *s, uint8_t flags) {
  ...
  static const uint8_t
      valid_flags[PICO_SOCKET_STATE_TCP_ARRAYSIZ][MAX_VALID_FLAGS] = {
          {
          ...
          {/* PICO_SOCKET_STATE_TCP_SYN_SENT   */ PICO_TCP_SYNACK, PICO_TCP_RST,
           PICO_TCP_RSTACK},
          {/* PICO_SOCKET_STATE_TCP_SYN_RECV   */ PICO_TCP_SYN, PICO_TCP_ACK,
           PICO_TCP_PSH, PICO_TCP_PSHACK, PICO_TCP_FINACK, PICO_TCP_FINPSHACK,
           PICO_TCP_RST},
          {/* PICO_SOCKET_STATE_TCP_ESTABLISHED*/ PICO_TCP_SYN, PICO_TCP_SYNACK,
           PICO_TCP_ACK, PICO_TCP_PSH, PICO_TCP_PSHACK, PICO_TCP_FIN,
           PICO_TCP_FINACK, PICO_TCP_FINPSHACK, PICO_TCP_RST, PICO_TCP_RSTACK},
          {/* PICO_SOCKET_STATE_TCP_CLOSE_WAIT */ PICO_TCP_SYNACK, PICO_TCP_ACK,
           PICO_TCP_PSH, PICO_TCP_PSHACK, PICO_TCP_FIN, PICO_TCP_FINACK,
           PICO_TCP_FINPSHACK, PICO_TCP_RST},
          {/* PICO_SOCKET_STATE_TCP_LAST_ACK   */ PICO_TCP_SYNACK, PICO_TCP_ACK,
           PICO_TCP_PSH, PICO_TCP_PSHACK, PICO_TCP_FIN, PICO_TCP_FINACK,
           PICO_TCP_FINPSHACK, PICO_TCP_RST},
          {/* PICO_SOCKET_STATE_TCP_FIN_WAIT1  */ PICO_TCP_SYNACK, PICO_TCP_ACK,
           PICO_TCP_PSH, PICO_TCP_PSHACK, PICO_TCP_FIN, PICO_TCP_FINACK,
           PICO_TCP_FINPSHACK, PICO_TCP_RST},
          {/* PICO_SOCKET_STATE_TCP_FIN_WAIT2  */ PICO_TCP_SYNACK, PICO_TCP_ACK,
           PICO_TCP_PSH, PICO_TCP_PSHACK, PICO_TCP_FIN, PICO_TCP_FINACK,
           PICO_TCP_FINPSHACK, PICO_TCP_RST},
          {/* PICO_SOCKET_STATE_TCP_CLOSING    */ PICO_TCP_SYNACK, PICO_TCP_ACK,
           PICO_TCP_PSH, PICO_TCP_PSHACK, PICO_TCP_FIN, PICO_TCP_FINACK,
           PICO_TCP_FINPSHACK, PICO_TCP_RST},
          {/* PICO_SOCKET_STATE_TCP_TIME_WAIT  */ PICO_TCP_SYNACK, PICO_TCP_ACK,
           PICO_TCP_PSH, PICO_TCP_PSHACK, PICO_TCP_FIN, PICO_TCP_FINACK,
           PICO_TCP_FINPSHACK, PICO_TCP_RST},
      };
  if (!flags) return 1;
  ...
  for (i = 0; i < MAX_VALID_FLAGS; i++) {
    if (valid_flags[s->state >> 8u][i] == flags) return 0;
  }
  return 1;
}

static void tcp_action_call(int (*call)(struct pico_socket *s,
                                        struct pico_frame *f),
                            struct pico_socket *s, struct pico_frame *f) {
  if (call) call(s, f);
}

static int tcp_action_by_flags(const struct tcp_action_entry *action,
                               struct pico_socket *s, struct pico_frame *f,
                               uint8_t flags) {
  int ret = 0;
  ...
  if ((f->payload_len > 0 || (flags & PICO_TCP_PSH)) &&
      !(s->state & PICO_SOCKET_STATE_CLOSED) &&
      !TCP_IS_STATE(s, PICO_SOCKET_STATE_TCP_LISTEN)) {
    ret = f->payload_len;
    dbg("tcp push action\n");
  ...
  if ((flags == (PICO_TCP_FIN | PICO_TCP_ACK)) ||
      (flags == (PICO_TCP_FIN | PICO_TCP_ACK | PICO_TCP_PSH))) {
...
int pico_tcp_input(struct pico_socket *s, struct pico_frame *f) {
  ...
  dbg("[sam] TCP> [tcp input] socket: %p state: %d <-- local port:%u remote "
      ...
      SEQN(f), ACKN(f), hdr->flags, f->transport_len, (hdr->len & 0xf0) >> 2,
      f->payload_len);
  ...
  if (s->ev_pending) tcp_wakeup_pending(s, s->ev_pending);
...
static struct pico_frame *tcp_split_segment(struct pico_socket_tcp *t,
                                            struct pico_frame *f,
                                            uint16_t size) {
  struct pico_frame *f1, *f2;
  uint16_t size1, size2, size_f;
  uint16_t overhead;
  struct pico_tcp_hdr *hdr1, *hdr2,
      *hdr = (struct pico_tcp_hdr *)f->transport_hdr;
  overhead = pico_tcp_overhead(&t->sock);
  size_f = f->payload_len;

  if (size >= size_f) return f; /* no need to split! */

  size1 = size;
  size2 = (uint16_t)(size_f - size);

  f1 = pico_socket_frame_alloc(&t->sock, get_sock_dev(&t->sock),
                               (uint16_t)(size1 + overhead));
  f2 = pico_socket_frame_alloc(&t->sock, get_sock_dev(&t->sock),
                               (uint16_t)(size2 + overhead));
...
int pico_tcp_output(struct pico_socket *s, int loop_score) {
  ...
  while ((f) && (t->cwnd >= t->in_flight)) {
     ...
     */
    if ((uint32_t)(seq_diff + f->payload_len) >
        (uint32_t)(t->recv_wnd << t->recv_wnd_scale)) {
      f = tcp_split_segment(t, f, (uint16_t)(t->recv_wnd << t->recv_wnd_scale));
      if (!f) break;

      /* Limit sending window to packets in flight (right sizing) */
      t->cwnd = (uint16_t)t->in_flight;
      if (t->cwnd < 1) t->cwnd = 1;
    }

    tcp_dbg(
        "TCP> DEQUEUED (for output) frame %08x, acks %08x len= %d, remaining "
        "frames %d\n",
...
static struct pico_frame *pico_hold_segment_make(struct pico_socket_tcp *t) {
  ...
  while ((f_temp != NULL) && ((total_len + f_temp->payload_len) <= t->mss)) {
    ...
    f_temp = next_segment(&t->tcpq_hold, f_temp);
    if (f_temp == NULL) break;
  }
  /* alloc new frame with payload size = off + total_len */
  f_new =
      pico_socket_frame_alloc(s, get_sock_dev(s), (uint16_t)(off + total_len));
  if (!f_new) {
    pico_err = PICO_ERR_ENOMEM;
    return f_new;
  }
  ...
  f_temp = first_segment(&t->tcpq_hold);
  hdr->seq = ((struct pico_tcp_hdr *)(f_temp->transport_hdr))
                 ->seq; /* get sequence number of first frame */
  hdr->trans.sport = t->sock.local_port;
  hdr->trans.dport = t->sock.remote_port;

  /* check till total_payload_len <= MSS */
  while ((f_temp != NULL) &&
         ((total_payload_len + f_temp->payload_len) <= t->mss)) {
    /* cpy data and discard frame */
    test++;
    memcpy(f_new->payload + total_payload_len, f_temp->payload,
           f_temp->payload_len);
...
static int pico_tcp_push_nagle_enqueue(struct pico_socket_tcp *t,
                                       struct pico_frame *f) {
  if (pico_enqueue_segment(&t->tcpq_out, f) > 0) {
    tcp_dbg_nagle(
        "TCP_PUSH - NAGLE - Pushing segment %08x, len %08x to socket %p\n",
...
static int pico_tcp_push_nagle_hold(struct pico_socket_tcp *t,
                                    struct pico_frame *f) {
  ...
  if ((total_len >= t->mss) &&
    ...
    if (pico_enqueue_segment(&t->tcpq_hold, f) > 0) {
      tcp_dbg_nagle(
          "TCP_PUSH - NAGLE - Pushed into hold, make new (enqueued frames out "
          "%d)\n",
          t->tcpq_out.frames);
    ...
    if (pico_enqueue_segment(&t->tcpq_hold, f) > 0) {
      tcp_dbg_nagle(
          "TCP_PUSH - NAGLE - Pushed into hold (enqueued frames out %d)\n",
          t->tcpq_out.frames);
      t->snd_last += f->payload_len; /* XXX  WATCH OUT */
      return f->payload_len;
    } else {
...
static int pico_tcp_push_nagle_on(struct pico_socket_tcp *t,
                                  struct pico_frame *f) {
  /* Nagle's algorithm enabled, check if ready to send, or put frame in hold
   * queue */
...
/* original behavior kept when Nagle disabled;
   Nagle algorithm added here, keeping hold frame queue instead of eg linked
   list of data */
int pico_tcp_push(struct pico_protocol *self, struct pico_frame *f) {
  ...
  hdr->len =
      (uint8_t)((f->payload - f->transport_hdr) << 2u | (int8_t)t->jumbo);

  if ((uint32_t)f->payload_len >
      (uint32_t)(t->tcpq_out.max_size - t->tcpq_out.size))
  ...
  if (!IS_NAGLE_ENABLED((&(t->sock)))) {
    ...
    if (pico_enqueue_segment(&t->tcpq_out, f) > 0) {
      tcp_dbg_nagle(
          "TCP_PUSH - NO NAGLE - Pushing segment %08x, len %08x to socket %p\n",
          t->snd_last + 1, f->payload_len, t);
      t->snd_last += f->payload_len;
      return f->payload_len;
    } else {
...
void pico_tcp_notify_closing(struct pico_socket *sck) {
  ...
  if (t->tcpq_out.frames == 0) {
    if (!checkLocalClosing(sck)) checkRemoteClosing(sck);
  }
}
