...
/*
#define PICO_TFTP_STATE_OPTIONS_RX  1
#define PICO_TFTP_STATE_RX          2
#define PICO_TFTP_STATE_RX_LAST     3
#define PICO_TFTP_STATE_OPTIONS_TX  4
#define PICO_TFTP_STATE_TX          5
#define PICO_TFTP_STATE_TX_LAST     6
#define PICO_TFTP_STATE_CLOSING     7
*/

/* NEW STATES... */
#define TFTP_SERVER_READ            0
#define TFTP_SERVER_WRITE           1
#define TFTP_STATE_READ_REQUESTED   2
#define TFTP_STATE_RX               3
#define TFTP_STATE_WRITE_REQUESTED  4
#define TFTP_STATE_TX               5
#define TFTP_STATE_WAIT_OPT_CONFIRM 6
#define TFTP_STATE_WAIT_LAST_ACK    7
#define TFTP_STATE_CLOSING          8
...
struct pico_tftp_session {
    ...
    void *argument;
    int (*callback)(struct pico_tftp_session *session, uint16_t event, uint8_t *block, uint32_t len, void *arg);
    uint32_t file_size;
...
struct automa_events {
    void (*ack)(struct pico_tftp_session *session, uint32_t len, union pico_address *a, uint16_t port);
    void (*data)(struct pico_tftp_session *session, uint32_t len, union pico_address *a, uint16_t port);
    void (*error)(struct pico_tftp_session *session, uint32_t len, union pico_address *a, uint16_t port);
    void (*oack)(struct pico_tftp_session *session, uint32_t len, union pico_address *a, uint16_t port);
...
static inline int do_callback(struct pico_tftp_session *session, uint16_t err, uint8_t *data, uint32_t len)
{
    int ret;
...
static void tftp_send(struct pico_tftp_session *session, int len)
{
    if (len)
        ...
        len = session->block_len;

    pico_socket_sendto(session->socket, session->tftp_block, session->block_len, &session->remote_address, session->remote_port);
}
...
static size_t prepare_options_string(struct pico_tftp_session *session, char *str_options, uint32_t filesize)
{
    size_t len = 0;
    int res;
...
static void tftp_send_error(struct pico_tftp_session *session, union pico_address *a, uint16_t port, uint16_t errcode, const char *errmsg)
{
    ...
    if (session) {
        ...
        (void)pico_socket_sendto(server.listen_socket, eh, (int) (len + sizeof(struct pico_tftp_err_hdr)), a, port);
}
...
static void tftp_send_data(struct pico_tftp_session *session, const uint8_t *data, uint32_t len)
{
    ...
    memcpy(session->tftp_block + sizeof(struct pico_tftp_data_hdr), data, len);
    pico_socket_sendto(session->socket, session->tftp_block, (int) (len + sizeof(struct pico_tftp_data_hdr)),
                       &session->remote_address, session->remote_port);
    tftp_schedule_timeout(session, PICO_TFTP_TIMEOUT);
}
...
static void tftp_req(uint8_t *block, uint32_t len, union pico_address *a, uint16_t port)
{
    ...
    case PICO_TFTP_WRQ:
        ...
        break;
    default:
        tftp_send_error(NULL, a, port, TFTP_ERR_EILL, "Illegal opcode");
    }
}

static int event_ack_base(struct pico_tftp_session *session, uint32_t len, union pico_address *a, uint16_t port)
{
    struct pico_tftp_data_hdr *dh;
    uint16_t block_n;
...
static inline int event_ack0_check(struct pico_tftp_session *session, uint32_t len, union pico_address *a, uint16_t port)
{
    struct pico_tftp_data_hdr *dh;
    uint16_t block_n;
    ...
    if (session->state == TFTP_STATE_WRITE_REQUESTED) {
        session->remote_port = port;
    }
...
static void event_ack0_wr(struct pico_tftp_session *session, uint32_t len, union pico_address *a, uint16_t port)
{
    if (!event_ack0_check(session, len, a, port))
        do_callback(session, PICO_TFTP_EV_OK, session->tftp_block, 0);
}

static void event_ack0_woc(struct pico_tftp_session *session, uint32_t len, union pico_address *a, uint16_t port)
{
    if (!event_ack0_check(session, len, a, port))
        do_callback(session, PICO_TFTP_EV_OPT, session->tftp_block, 0);
}

static void event_ack(struct pico_tftp_session *session, uint32_t len, union pico_address *a, uint16_t port)
{
    if (!event_ack_base(session, len, a, port))
        do_callback(session, PICO_TFTP_EV_OK, session->tftp_block, 0);
}

static void event_ack_last(struct pico_tftp_session *session, uint32_t len, union pico_address *a, uint16_t port)
{
    if (!event_ack_base(session, len, a, port))
        tftp_finish(session);
}

static void event_data(struct pico_tftp_session *session, uint32_t len, union pico_address *a, uint16_t port)
{
    struct pico_tftp_data_hdr *dh;
...
static void event_err(struct pico_tftp_session *session, uint32_t len, union pico_address *a, uint16_t port)
{
    (void)a;
    (void)port;
    do_callback(session, PICO_TFTP_EV_ERR_PEER, session->tftp_block, len);
...
static inline void event_oack(struct pico_tftp_session *session, uint32_t len, union pico_address *a, uint16_t port)
{
...
static void event_oack_rr(struct pico_tftp_session *session, uint32_t len, union pico_address *a, uint16_t port)
{
    event_oack(session, len, a, port);
    session->state = TFTP_STATE_RX;
}

static void event_oack_wr(struct pico_tftp_session *session, uint32_t len, union pico_address *a, uint16_t port)
{
    event_oack(session, len, a, port);
    session->state = TFTP_STATE_TX;
}
...
void unexpected(struct pico_tftp_session *session, uint32_t len, union pico_address *a, uint16_t port)
{
    (void)len;
    tftp_send_error(session, a, port, TFTP_ERR_EILL, "Unexpected message");
}

void null(struct pico_tftp_session *session, uint32_t len, union pico_address *a, uint16_t port)
{
    (void)session;
    (void)len;
    (void)a;
...
static struct automa_events fsm[AUTOMA_STATES] = {
    ...
    /* ************************************************************************************************************* */
    { /* TFTP_STATE_SERVER_READ      */ null,           null,       null,       null,           NULL},
    { /* TFTP_STATE_SERVER_WRITE     */ null,           null,       null,       null,           NULL},
    { /* TFTP_STATE_READ_REQUESTED   */ unexpected,     event_data, event_err,  event_oack_rr,  event_timeout},
...
static void tftp_message_received(struct pico_tftp_session *session, uint8_t *block, uint32_t len, union pico_address *a, uint16_t port)
{
...
static void tftp_cb(uint16_t ev, struct pico_socket *s)
{
    ...
    if (session) {
        if (ev == PICO_SOCK_EV_ERR) {
            ...
            do_callback(session, PICO_TFTP_EV_ERR_LOCAL, session->tftp_block, (uint32_t)strlen((char *)session->tftp_block));
            tftp_finish(session);
            return;
        }
        r = pico_socket_recvfrom(s, session->tftp_block, PICO_TFTP_BLOCK_SIZE, &ep, &port);
        if (r < (int)sizeof(struct pico_tftp_hdr))
            return;

        tftp_message_received(session, session->tftp_block, (uint32_t)r, &ep, port);
    } else {
        if (!server.listen_socket || s != server.listen_socket) {
            return;
        }
        r = pico_socket_recvfrom(s, server.tftp_block, PICO_TFTP_BLOCK_SIZE, &ep, &port);
            if (r < (int)sizeof(struct pico_tftp_hdr))
                return;
        tftp_req(server.tftp_block, (uint32_t)r, &ep, port);
    }
}
...
int pico_tftp_set_option(struct pico_tftp_session *session, uint8_t type, uint32_t value)
{
    ...
    case PICO_TFTP_OPTION_TIME:
        ...
        session->option_timeout = value & 0xFF;
...
int pico_tftp_parse_request_args(char * args, uint32_t len, int * options, uint8_t * timeout, uint32_t * filesize)
{
    ...
    return parse_optional_arguments(args, (size_t)(end_args - args), options, timeout, filesize);
}
